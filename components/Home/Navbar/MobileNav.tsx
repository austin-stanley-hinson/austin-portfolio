...
const MobileNav = ({ closeNav, showNav }: Props) => {
  ...
  return (
    <div>
      {/*Overlay*/}
      <div className={`fixed inset-0 ${sidebarOpenClose} transform transition-all duration-500 z-1002 bg-black
    opacity-70 w-full h-screen`}>
      ...
      <div className={`text-white ${sidebarOpenClose} fixed justify-center flex flex-col h-full transform transition-all duration-500 delay-300 w-[80%] sm:w-[60%] bg-purple-700 space-y-6 z-1050`}>
        ...
        <X onClick = {closeNav} className="absolute top-[0.7rem] right-[1.4rem] sm:h-8 w-6 h-6" />
